...
form.addEventListener('submit', e => {
  ...
  // Verificamos si el valor es un número válido y está en el rango de 0 a 9
  const isValidValue = !isNaN(value) && value >= 0 && value <= 9;

  // Usamos el valor válido o un texto aleatorio
  const tempText = isValidValue 
    ? text.slice(0, value).map(item => `<p class="result">${item}</p>`).join("") 
